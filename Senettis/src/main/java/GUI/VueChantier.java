...
public class VueChantier {
	...
	/***
	 * Pour créer une vueChantier : dispose si une vueChantier existe deja, creer le composite et lui affecte le layout RowLayout Vertical
	 * Appelle ensuite les fonctions compositeSelectionCreer et vueChantierAfficher
	...
	public void newVueChantier (Composite composite) {
		...
		vue.pack(); selection.pack(); vueChantier.pack();
	...
	/***
	 * Va modifier la partie Vue (partie inferieure droite) et y ajoutant le formulaire de modification d'un chantier
	 */
		...
		buttonValidation.addSelectionListener(new SelectionAdapter() {
		  ...
		  @Override public void widgetSelected(SelectionEvent arg0) {
			   ...
			   try { 
				   ...
				   selectedChantier = new Chantier(id, textNom.getText(), textAdresse.getText(), Double.parseDouble(textCA.getText()), "Publié");
				   validerModification();
			    } catch (Throwable e) { 
			    	e.printStackTrace(); 
			    	...
			    	dialog.setMessage("Une erreur est survenue lors de la modification du chantier. "+'\n'+e.getMessage());
			    	dialog.open();
			    } 
		  ...
		  @Override public void widgetSelected(SelectionEvent arg0) {
			  ...
			  try { 
				  validerCreation(textNom.getText(), textAdresse.getText(), textCA.getText());   
			    	...
			    	dialog.setMessage("Une erreur est survenue lors de la création d'un chantier. "+'\n'+e.getMessage());
			    	dialog.open();
			    } 
		  }
		});
    ...
    //--------------------------------------------------------------------------
	/***
	 * affiche le tableau avec tous les chantiers dans la base de donnees dont le status est publie
	 */
		...
		table.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				if (table.getSelectionIndex() != -1) {
					...
					selection.dispose();
					try {
						selectedChantier = Chantier.getChantierById(Integer.parseInt(table.getSelection()[0].getText(3)));
				...
				else { // si plus rien n'est selectionner on passe selectedChantier a null et on enleve le menu du clic droit et on enleve les boutons pour modifier et supprimer
