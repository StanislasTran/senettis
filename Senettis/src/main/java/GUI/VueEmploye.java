...
public class VueEmploye {
	...
	public void amortiSelection() {
		...
		boutonCreation.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				selectedEmploye = null; selectedAmorti = null;
				vueCreationA();
			}
		});
		...
		if (selectedAmorti != null) {
			...
			boutonSupp.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent arg0) {
					try {
						AmmortissementEmploye ae = AmmortissementEmploye.getAmmortissementEmployeById(selectedAmorti.getAmmortissementEmployeId());
						...
						dialog.setMessage("Voulez vous supprimer le coût à amortir de l'employé " + Employee.getEmployeById(ae.getEmployeId()).getNom() + " " + Employee.getEmployeById(ae.getEmployeId()).getPrenom()+ " ?");
	...
	public void vueFormulaireAmorti() {
		...
		if (selectedEmploye != null){
			try {
				if (selectedEmploye.getNameString().length() > 25) {
					employes.setText(selectedEmploye.getNameString().substring(0, 23)+"..."+";id:"+((Integer)selectedEmploye.getEmployeId()).toString());
				}
				else {
					employes.setText(selectedEmploye.getNameString()+" ; id :"+((Integer)selectedEmploye.getEmployeId()).toString());
				}
			} catch (Exception e1) {
				e1.printStackTrace(); 
			...
			for (Employee e : Employee.getAllEmploye()) {
				if (e.getStatus() == "Publié") {
					if (e.getNameString().length() > 25) {
						employes.add(e.getNameString().substring(0, 23)+"..."+";id:"+((Integer)e.getEmployeId()).toString());
					}
					else {
		...
		buttonAnnulation.addSelectionListener(new SelectionAdapter() {
			...
			@Override
			public void widgetSelected(SelectionEvent arg0) {
				selectedAmorti = null; selectedEmploye = null;
				vueAmortissement();
			}
		});
		...
		tableAmorti.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				if (tableAmorti.getSelectionIndex() != -1) {
					...
					try {
						selectedAmorti = AmmortissementEmploye.getAmmortissementEmployeById(Integer.parseInt(tableAmorti.getSelection()[0].getText(0)));
					} catch (NumberFormatException | SQLException e1) {
	...
	public void vueCouts() {
		...
		String[] titles = {"Id", "Nom" ,"Prenom","Matricule" ,"Période", "salaire Net","salaire Brut","nombreHeures",
				"charges patronales","masse salariale","Transport","Telephone","mutuelle","paniers", "prets","saisie arret"};
		for (String title : titles) {
	...
	public void showFormulaire() {
		...
		if (selectedEmploye == null) {
			titre = "";nom=""; prenom=""; mail=""; telephone=""; numeroMatricule=""; pointure=""; taille=""; dateArrivee="";
		}
		else {
			titre = selectedEmploye.getTitre();nom=selectedEmploye.getNom(); prenom=selectedEmploye.getPrenom(); mail=selectedEmploye.getMail();
			telephone=selectedEmploye.getTelephone(); numeroMatricule=selectedEmploye.getNumeroMatricule().toString(); pointure=selectedEmploye.getPointure();
			taille=selectedEmploye.getTaille(); dateArrivee=selectedEmploye.getDateArrivee();
		}
	...
	public void vueEmployeAfficher() {
		...
		String[] titles = { "Titre  ", "Nom        ", "Prenom     ", "Email             ", "Téléphone", "N° de matricule", "Pointure", "Taille",
				"Date d'ancienneté", "Ancienneté", "Id DB"};
		for (String title : titles) {
