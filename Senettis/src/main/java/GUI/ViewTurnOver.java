...
public class ViewTurnOver {
		...
		tabFolder.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent event) {
				if (tabFolder.getSelection()[0].equals(tabLivraison)) {
					...
					RowLayout rowLayout = new RowLayout();
					rowLayout.type = SWT.VERTICAL;
					vueAutres = new Composite(tabFolder, SWT.NONE);
				...
				public void widgetSelected(SelectionEvent arg0) {
					try {
						...
						dialog.setMessage("Voulez vous supprimer le coût à amortir du chantier "
								+ Site.getSiteById(ae.getChantierId()).getName() + " ?");
	...
	public void vueFormulaireAmorti() {
		...
		try {
			for (Site e : Site.getAllChantier()) {
				...
				if (e.getStatus().getValue().equals("Publié")) {
	...
	public void updateTableAmorti() {
		...
		try {
			for (AmmortissementChantier a : AmmortissementChantier.getAllAmmortissementChantier()) {
				// on verifie le status
				if (a.getStatus().equals("Publié") && selectedChantier.getSiteId() == a.getChantierId()) {
	...
	public void formulaireModification() {
		...
		try {
			if (Site.getSiteById(selectedLivraison.getIdChantier()).getStatus().getValue().equals("Publié")) {
		...
		try {
			for (Site c : Site.getAllChantier()) {
				if (c.getName() != Site.getSiteById(selectedLivraison.getIdChantier()).getName()
						&& c.getStatus().getValue().equals("Publié")) {
		...
		// buttonCalculPrix.setBounds(10, 60, 200, 30); // -> ne focntionne pas en
		// fillLayout
	...
	public void formulaireCreation() {
		...
		try {
			for (Site c : Site.getAllChantier()) {
				if (c.getStatus().getValue().equals("Publié")) {
